...
def screen_onboard1():
    ...
    <div class="mk4-wiz">
        <div class="mk4-card">
            <div class="mk4-wizhead"><span class="k-logo">trdr</span>
                <span class="mk4-step">처음 설정 <b>1/3</b> — 수집기 연결</span></div>
            <div class="mk4-lede">기본 데이터 수집에 API 키가 필요합니다. 키는 각 기관에서 무료로 발급합니다.</div>
            {rows}
            ...
            <div class="mk4-wizfoot">
                <span class="k-dim" style="font-size:10.5px">필수(KIS)가 검증되면 넘어갈 수 있습니다</span>
                <button class="k-btn k-btn--amber" data-goto="onboard2">다음 — 데이터 셋업</button>
            </div>
        </div>
    </div>"""
    return frame(
        "onboard1", "온보딩 1/3 — 수집기 연결", "VW-ONBOARD · 결정 01kzdz3s·01kzdr99",
        f'<div class="k-app mk-desk">{card}</div>',
        "실행 직후의 첫 화면. 기본 데이터 수집용 키 연결이 모든 것에 선행한다 — KIS만 필수이고, "
        "검증은 저장이 아니라 <b>테스트 호출</b>이다. 화면은 고정 표가 아니라 설치된 수집기 "
        "레지스트리를 렌더링한다: 서드파티 수집기를 설치하면 같은 화면에 나타나고, 내장 3개도 "
        "같은 표면 위의 플러그인이다. keychain 안내가 이 화면의 고정 요소다 — 키를 건네는 바로 "
        "그 순간이 local-first를 말할 자리이기 때문이다.")
...
def screen_onboard2():
    ...
    <div class="mk4-wiz">
        <div class="mk4-card">
            <div class="mk4-wizhead"><span class="k-logo">trdr</span>
                <span class="mk4-step">처음 설정 <b>2/3</b> — 데이터 셋업</span></div>
            <div class="mk4-sect">기본 포함 — 설치에 이미 들어 있습니다</div>
            {bundled}
            ...
            <div class="mk4-wizfoot">
                <span class="k-dim" style="font-size:10.5px">시작 후 언제든 설정에서 수집기를 더할 수 있습니다</span>
...
.mk4-wiz { flex: 1; display: flex; align-items: center; justify-content: center; }
.mk4-card { width: 640px; background: var(--k-bg-1); border: 1px solid var(--k-line-strong); padding: 26px 30px; display: flex; flex-direction: column; gap: 12px; }
.mk4-wizhead { display: flex; align-items: baseline; gap: 14px; margin-bottom: 4px; }
.mk4-step { font-size: 12px; color: var(--k-ink-2); }
.mk4-step b { color: var(--k-amber); font-family: var(--k-mono); }
.mk4-lede { font-size: 12px; color: var(--k-ink-2); line-height: 1.7; }
.mk4-sect { font: 700 10.5px var(--k-sans); color: var(--k-ink-3); text-transform: none; letter-spacing: .04em; margin-top: 8px; border-bottom: 1px solid var(--k-line); padding-bottom: 5px; }
.mk4-colrow { display: flex; align-items: center; gap: 12px; padding: 9px 2px; border-bottom: 1px solid var(--k-line); font-size: 12px; }
.mk4-colrow:hover { background: var(--k-bg-2); }
.mk4-colname { width: 128px; flex: none; display: flex; gap: 6px; align-items: center; font-weight: 700; }
.mk4-coldata { flex: 1; color: var(--k-ink-2); font-size: 11px; line-height: 1.5; }
.mk4-colkey { flex: none; display: flex; gap: 6px; align-items: center; }
.mk4-key { border: 1px solid var(--k-line-strong); background: var(--k-bg-0); padding: 5px 9px; font-size: 11px; color: var(--k-ink-2); }
.mk4-plugin { border-style: dashed; border-color: var(--k-line); }
.mk4-trust { border: 1px solid var(--k-amber-dim); background: var(--k-amber-bg); padding: 11px 14px; font-size: 11.5px; line-height: 1.7; color: var(--k-ink-2); }
.mk4-trust b { color: var(--k-ink); }
.mk4-wizfoot { display: flex; align-items: center; justify-content: space-between; margin-top: 6px; }
...
PRESET_NOTES = """
    { screen: "onboard1", x: 50, y: 74, text: "keychain 안내는 이 화면의 고정 요소다(결정 01kzdz3s) — 키를 건네는 바로 그 순간이 local-first 신뢰 명제를 말할 자리다. 값은 화면·로그 어디에도 안 나온다." },
    { screen: "onboard1", x: 50, y: 56, text: "이 표는 고정이 아니라 설치된 수집기 레지스트리의 렌더링이다(결정 01kzdr99). KRX처럼 id/pw가 필요한 채널은 공식 지원하지 않는다(결정 01kzdr2d) — 그 몫은 플러그인 생태계다." },
